...
<script>
import MessageComposer from "@/client/components/Basket/message-composer/MessageComposer";
import MessageFeed from "@/client/components/Basket/message-feed/MessageFeed";
import ConversationHeader from "@/client/components/Basket/conversation/ConversationHeader";
...
name: "Conversation",
  ...
  methods:{
    sendMessage(text){
      this.$emit('message', {user: this.contact._id, text: text, isIncoming: false })
    }
  }
}
</script>
