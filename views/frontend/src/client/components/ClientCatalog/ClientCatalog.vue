    ...
    <div id="categories" class="pt-3 col-lg-3 col-md-4">
      <div class="catalog-left">
        <div class="sticked-categories" style="height: 300px;overflow: auto;">
          <h3 class="price">Категории:</h3>
          <ul class="list-group">
            <li v-for="category in listCategory" :key="category._id"
              :class="{ active: category._id === selectedCategory }" class="catalog-list"
              @click="setCategory(category._id)">{{ category.name }}</li>
          </ul>
        </div>
      </div>
      <div class="mobile-btns">
        <button class="save" @click="showFilterCategory('category')"><img class="list-icon"
            src="../../../assets/icons/list.svg"><span>Выбрать категорию</span></button>
        <div class="showCategory">
          <div class="mobile-header d-flex justify-content-between align-items-center">
            <h3 class="filter-category">Категории</h3>
        ...
        <div class="d-flex mb-3">
          <button class="app-buttons-item" @click="showFilterCategory('filter')"><img
              src="../../../assets/icons/filter-btn.svg"><span>Фильтры</span></button>
        </div>
        <div class="showFilter">
          <div class="mobile-header d-flex justify-content-between align-items-center">
            <h3 class="filter-category">Фильтр</h3>
            <img @click="removeFilterCategory('filter')" class="close-nav" src="../../../assets/icons/xBlack.svg">
          </div>
          <div class="d-flex mb-5">
            <div style="width:40%" class="mr-3">
              <label class="label-client">От</label>
              <input class="client-input" v-model="from" style="width:100%">
            </div>
            <div style="width:40%">
              <label class="label-client">До</label>
        ...
        <div class="category-key">Категории: <span class="category-value">{{ showCategory }}</span></div>
    ...
    <div id="products" class="pt-3 col-lg-9 col-md-8">
      <div v-if="filteredList.length === 0 && searchText !== '' && searchStatus">
        <div style="display: flex;justify-content: center;align-items: center;">
          Результаты не найдены для {{ searchText }}
        </div>
      </div>

      <div v-if="filteredList.length !== 0 && searchText !== '' && searchStatus">
        <div style="display: flex;justify-content: center;align-items: center;">
          Результаты: {{ searchText }}
        </div>
      </div>
      ...
      <div style="display: flex;height: 50px;width: 100%;justify-content: center;align-items: center;">
        ...
        <div class="paginationItem" v-if="currentPage > 3" @click="currentPage = 1; getProducts()" style="cursor: pointer;">1
        </div>
        <div v-if="currentPage > 3">
          ...
        </div>
        <div v-bind:class="{ activePage: currentPage === page2 }" class="paginationItem"
          v-for="page2 in numberOfPagesArray.filter(num => num < currentPage + 3 && num > currentPage - 3)" :key="page2"
          @click="setPage(page2)">
          {{ page2 }}
        </div>
