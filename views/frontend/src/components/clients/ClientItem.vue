<template>
  <div v-for="client in clientList" class="table-item d-flex align-items-center justify-content-between" :key="client._id">
      ...
      <div class="table-child" v-show="data_check.birthday_checked" style="width: 12%;">{{client.birthDate ? client.birthDate.slice(0,10) :  ''}}</div>
      <div class="table-child" v-show="data_check.discount_checked" style="width: 10%;">10%</div>
      <div class="table-child" v-if="client.category"  style="width: 14%;">{{client.category.name}}</div>
