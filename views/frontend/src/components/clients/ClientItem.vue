<template>
  <div v-for="client in clientList" class="table-item d-flex align-items-center justify-content-between" :key="client._id">
      ...
      <div class="table-child" v-if="data_check.birthday_checked && client.birthDate !== null" style="width: 12%;">{{client.birthDate.slice(0,10)}}</div>
      <div class="table-child" v-show="data_check.discount_checked" style="width: 10%;">10%</div>
      <div class="table-child"  style="width: 14%;">
        <div  v-if="client.category === undefined">
          no category
        </div>
        <div v-else>
      ...
      <div class="table-child" v-if="data_check.register_date_checked && client.createdAt !== null"  style="width: 18%;">{{client.createdAt.slice(0,10)}}</div>
