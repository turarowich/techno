<template>
    <div>
     ...
     <div v-else>
       <div  v-for="order in orderList" class="table-item d-flex justify-content-between align-items-center" :key="order._id">
         ...
         <div v-if="order" class="table-child d-flex align-items-center"  style="width: 25%;">
           <div  class="table-img">
             <img  v-if="order && order.img" :src="imgSrc+'/'+order.img">
             <img v-else src="../../assets/icons/no-catalog.svg">
           </div>
           <span style="overflow: hidden;text-overflow: ellipsis;">{{order.name ? order.name : 'empty'}}</span>
         </div>
         ...
         <div class="table-child"  style="width: 10%;">{{totalSample(order).total}} {{catalog_settings.currency}}</div>
         <div class="table-child"  style="width: 10%;">{{totalSample(order).income}} {{catalog_settings.currency}}</div>
         <div class="table-child"  style="width: 10%;">{{totalSample(order).result}} {{catalog_settings.currency}}</div>
           ...
           <div class="dropleft dropMenu">
             ...
             <div class="dropdown-menu" aria-labelledby="dropdownMenuTotal">
               <ul class="list-group " >
                 ...
                 <li v-if="![3].includes(order.status) && check('canChangeOrderStatus', null, null) && order.status != 2" class="list-group-item" @click="statusChange(order, 3)">Отмена</li>
               </ul>
             </div>
           </div>
         </div>
  ...
  export default {
    ...
    methods: {
      totalSample(order) {
        const sample = order.details[0]
        const totalSample = sample.sewing + sample.kroy + sample.overlok + sample.rasposhivalka + sample.vengerka + sample.beltclipper + sample.petla + sample.iron + sample.otk + sample.package + sample.buttons + sample.technolog + sample.consumption + sample.furnitura
        return {
            total: totalSample,
