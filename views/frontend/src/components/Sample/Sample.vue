<template>
    <div class="orders">
      ...
      <div class="d-flex justify-content-between app-buttons" style="flex-wrap: wrap;">
        <div class="d-flex align-items-center" style="flex-wrap: wrap;">
          <button v-if="check()" class="app-buttons-item adding-btns" data-toggle="modal" @click="unSetSelectedOrder" data-target="#add-order" ><span>+ Добавить образец</span></button>
      ...
      <div class="d-flex main-content-header justify-content-between align-items-center">
        <div class="table-head d-flex align-items-center" style="width: 18%;">
          ...
          ID</div>
        <div class="table-head" style="width: 25%;">Продукт</div>
        ...
        <div class="table-head table-link d-flex align-items-center" style="width: 10%;" @click="sortByTotal()" >Cебестоимость <img class="total-pol" style="margin-left:7px" src="../../assets/icons/polygon.svg"></div>
        <div class="table-head" style="width: 10%;">Маржа</div>
        <div class="table-head" style="width: 10%;">Сумма</div>
  ...
  import AddOrder from "@/modals/sample/AddSample";
    ...
    methods: {
      ...
      selectOrder(id){
        ...
        this.orderList.map((item)=>{
          if(item._id === id) {
            ...
            console.log('order:::',item.client)
            // this.addNewProperty(this.select_order.products, "_id", 0, 'product')
          }
        })
      },
      getOrders(){
        this.axios.get(this.url('getSamplesAll')+'?populate=client')
        .then((response)=>{
      ...
      deleteOrder(id) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            this.axios.delete(this.url('deleteSample',id))
            .then(()=>{
              this.getOrders()
              this.$successAlert("Sample deleted")
            }).catch((error)=>{
              if(error.response && error.response.data){
                  this.$warningAlert(error.response.data.msg)
              }
