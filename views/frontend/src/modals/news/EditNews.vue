<template>
  <div class="modal fade right "  id="edit-news" tabindex="-1" role="dialog" aria-labelledby="edit-news" aria-hidden="true">
...
export default {
    ...
    watch: {
        selectedNews: function (val) {
            ...
            if (this.news.category == "promotions"){
                ...
                this.news.startDate = this.news.startDate.slice(0, 10)
                this.news.endDate = this.news.endDate.slice(0, 10)
            }else{
                this.news.startDate = ""
                this.news.endDate = ""
                this.news.category = false
            }
            this.newImage = false
        }
    },
    ...
    uploadPhoto(event) {
        ...
        if(event.target.files[0] && event.target.files[0].size > 3000000){
          ...
          console.log("edit" , this.news.img)
        }
    ...
    onSubmit(){
        ...
        this.axios.put(this.url('updateNews', news._id), form)
            .then(() => {
                this.$emit('getNews')
                setTimeout(function(){ 
